...
function CommentsItem() {
  ...
  return (
    <div>
      <h1>
        ({comment.id}) Comment title - {comment.name}
      </h1>
      <button onClick={deleteCommentHandler} value={comment.id}>
        Delete comment
      </button>
      <button>
        <Link to={`/Comments/edit/${comment.id}`}>Edit comment</Link>
      </button>
      <h3>
        Post: <Link to={`/Posts/${comment.postId}`}>{comment.post.title}</Link>
      </h3>
